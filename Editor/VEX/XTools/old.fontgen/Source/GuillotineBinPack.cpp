...
void CountNumFitting(const BPRect &freeRect, int width, int height, const std::vector<RectSize> &rects, 
...
void GuillotineBinPack::InsertMaxFitting(std::vector<RectSize> &rects, std::vector<BPRect> &dst, bool merge, 
	FreeRectChoiceHeuristic rectChoice, GuillotineSplitHeuristic splitMethod)
{
	dst.clear();
	int bestRect = 0;
	...
	while(rects.size() > 0 && freeRectangles.size() > 0)
	{
		...
		BPRect &freeRect = freeRectangles[0];

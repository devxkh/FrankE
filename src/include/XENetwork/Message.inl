...
template<typename T, typename std::enable_if<std::is_same<T, std::forward_list<typename T::value_type>>::value, int>::type = 0>
 void InsertAtEnd( T& container, typename T::value_type&& value ) {
...
template<std::size_t S = 0, typename... T>
 typename std::enable_if<S == sizeof...(T), Message&>::type operator<<( Message& message, const std::tuple<T...>& input ) {
	return message;
}
...
template<std::size_t S = 0, typename... T>
 typename std::enable_if<S == sizeof...(T), Message&>::type operator>>( const std::tuple<T...>& input, Message& message ) {
	return message;
}
...
template<typename T, typename std::enable_if<std::is_trivial<T>::value, int>::type = 0>
 Message& operator<<( Message& message, const T& input ) {
...
template<typename T, typename std::enable_if<std::is_trivial<T>::value, int>::type = 0>
 Message& operator>>( const T& input, Message& message ) {
	std::vector<char> data( sizeof( T ) );
...
template<typename T, std::size_t N>
 Message& operator<<( Message& message, const std::array<T, N>& input ) {
	for( const auto& e : input ) {
		message << e;
	}
...
template<typename T, std::size_t N, typename std::enable_if<std::is_trivial<T>::value, int>::type = 0>
 Message& operator>>( const std::array<T, N>& input, Message& message ) {
	for( const auto& e : input ) {
		e >> message;
	}
...
/// @cond
template<std::size_t S = 0, typename... T>
 typename std::enable_if<S == sizeof...(T), Message&>::type operator>>( Message& message, std::tuple<T...>& output ) {
	return message;
}
...
template<typename T, typename std::enable_if<std::is_trivial<T>::value, int>::type = 0>
 Message& operator>>( Message& message, T& output ) {
...
/* Dangerous... doesn't do what you expect...
template<typename T, typename std::enable_if<std::is_trivial<T>::value, int>::type = 0>
 Message& operator<<=( T& output, Message& message ) {
	assert( message.GetSize() >= sizeof( T ) );
...
template<typename T, std::size_t N>
 Message& operator>>( Message& message, std::array<T, N>& output ) {
...
/* Dangerous... doesn't do what you expect...
template<typename T, std::size_t N>
 Message& operator<<=( std::array<T, N>& output, Message& message ) {
	for( std::size_t index = 0; index < N; ++index ) {
		T value;
		value <<= message;
		output[index] = std::move( value );

#include <XEScripts/XML/DotSceneLoader.hpp>
#include <Ogre/OgreMain/include/Ogre.h>
#include <Ogre/Components/Terrain/include/OgreTerrain.h>
#include <Ogre/Components/Terrain/include/OgreTerrainGroup.h>
#include <Ogre/Components/Terrain/include/OgreTerrainMaterialGeneratorA.h>

#include <Ogre/Components/PagedGeometry/include/PagedGeometry.h>
#include <Ogre/Components/PagedGeometry/include/GrassLoader.h> 
#include <Ogre/Components/PagedGeometry/include/BatchPage.h>
#include <Ogre/Components/PagedGeometry/include/ImpostorPage.h>
#include <Ogre/Components/PagedGeometry/include/TreeLoader3D.h>
...
void DotSceneLoader::processTerrainPage(rapidxml::xml_node<>* XMLNode)
{
	...
	//Bugfix -> http://forum.ogitor.org/viewtopic.php?f=5&t=872
	if (XMLNode->first_attribute("pagedGeometryPageSize"))
		mPGPageSize = Ogre::StringConverter::parseInt(XMLNode->first_attribute("pagedGeometryPageSize")->value());
	else
		mPGPageSize = 0;

	if (XMLNode->first_attribute("pagedGeometryDetailDistance"))
		mPGDetailDistance = Ogre::StringConverter::parseInt(XMLNode->first_attribute("pagedGeometryDetailDistance")->value());
	else
		mPGDetailDistance = 0;
	
	//old mPGPageSize       = StringConverter::parseInt(XMLNode->first_attribute("pagedGeometryPageSize")->value());
	//old mPGDetailDistance = StringConverter::parseInt(XMLNode->first_attribute("pagedGeometryDetailDistance")->value());
	/*mPGPageSize = Ogre::StringConverter::parseInt(XMLNode->first_attribute("pagedGeometryPageSize")->value());
	mPGDetailDistance = Ogre::StringConverter::parseInt(XMLNode->first_attribute("pagedGeometryDetailDistance")->value());*/
	// error checking
	if (mPGPageSize < 10){
...
void DotSceneLoader::processGrassLayers(rapidxml::xml_node<>* XMLNode)
{
	...
	//todo ?? mGrassLoaderHandle->setVisibilityFlags(mTerrainGlobalOptions->getVisibilityFlags());
	...
	//todo ??	gLayer->setId(Ogre::StringConverter::parseInt(pElement->first_attribute("id")->value()));
	//todo ??	gLayer->setEnabled(Ogre::StringConverter::parseBool(pElement->first_attribute("enabled")->value()));
...
void DotSceneLoader::processLight(rapidxml::xml_node<>* XMLNode, Ogre::SceneNode *pParent)
{
	...
	pElement = XMLNode->first_node("position");
	if (pElement)
		pLight->getParentSceneNode()->setPosition(parseVector3(pElement));

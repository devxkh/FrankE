...
#include <Ogre/OgreMain/include/OgreVector4.h>
...
namespace rj
{
	...
	public:
		void load(GLTFScene *scene, const std::string &fn) const
		{
			...
			std::unordered_map<uint32_t, Ogre::Matrix4> meshId2Transform;
	...
	private:
		void parseSceneHierarchy(std::unordered_map<uint32_t, Ogre::Matrix4> &meshId2Transform, const picojson::array &nodes) const
		{
			...
			for (const auto &node : nodes)
			{
				...
				if (fields.find("rotation") != fields.end())
				{
					...
					rot = Ogre::Quaternion(q[3].get<double>(), q[0].get<double>(), q[1].get<double>(), q[2].get<double>());
				}

				Ogre::Vector3 scale(1.f);
				if (fields.find("scale") != fields.end())
				{
					const auto &v3 = fields.at("scale").get<picojson::array>();
					scale = Ogre::Vector3(v3[0].get<double>(), v3[1].get<double>(), v3[2].get<double>());
				}

				Ogre::Matrix4 I; // identity
				//n.local2parent = glm::translate(I, trans) * Ogre::Matrix4_cast(rot) * glm::scale(I, scale);
				I.setScale(scale);
				I.setTrans(trans);
				n.local2parent = I * Ogre::Matrix4(rot);
			...
			std::function<void (uint32_t, Ogre::Matrix4)> visit = [&visit, &meshId2Transform, &ns](uint32_t root, Ogre::Matrix4 T)
			{
				const auto &n = ns[root];
				auto test = T * n.local2parent;
				if (n.mesh != std::numeric_limits<uint32_t>::max()) meshId2Transform[n.mesh] = test;
		...
		void parseMeshes(std::vector<GLTFMesh> &ms, const picojson::array &meshes,
			...
			const std::unordered_map<uint32_t, Ogre::Matrix4> &meshId2Transform) const
		{
			...
			for (uint32_t meshId = 0; meshId < meshes.size(); ++meshId)
			{
				...
				Ogre::Matrix4 T = meshId2Transform.find(meshId) != meshId2Transform.end() ? meshId2Transform.at(meshId) : Ogre::Matrix4();
				//Ogre::Matrix4 Tit = glm::transpose(glm::inverse(T));
				Ogre::Matrix4 Tit = T.inverse().transpose();
				...
				for (const auto &prim : prims)
				{
					...
					// Positions
					{
						...
						Ogre::Vector3 *positions = reinterpret_cast<Ogre::Vector3 *>(&m.positions[posStart]);
						for (uint32_t i = 0; i < acc.count; ++i)
						{
							positions[i] = Ogre::Vector3(T * Ogre::Vector3(positions[i].x, positions[i].y, positions[i].z));
						//KH	positions[i] = Ogre::Vector3(T * Ogre::Vector3(positions[i].x, positions[i].y, positions[i].z, 1.f));
						}
					}
					// Normals
					{
						...
						Ogre::Vector3 *normals = reinterpret_cast<Ogre::Vector3 *>(&m.normals[nrmStart]);
						for (uint32_t i = 0; i < acc.count; ++i)
						{
							normals[i] = Ogre::Vector3(Tit * Ogre::Vector3(normals[i].x, normals[i].y, normals[i].z)).normalise();
						//		normals[i] = glm::normalize(Ogre::Vector3(Tit * Ogre::Vector4(normals[i], 0.f)));
					}
					}
					// Texture coordinates
					{

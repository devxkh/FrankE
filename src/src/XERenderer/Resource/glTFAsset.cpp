...
namespace gltf {
	...
	void* getBufferViewData(const Asset& asset, const Accessor& accessor) {
		...
		const Buffer& buffer = asset.buffers[bufferView.buffer];
	...
	void* generateNormals(float* positions, uint32_t accessorCount) {
		Ogre::Vector3* data = new Ogre::Vector3[accessorCount];

		uint32_t trianglesCount = accessorCount / 3;
		uint32_t positionsIdx = 0;
		for (uint32_t i = 0; i < trianglesCount; ++i) {
			Ogre::Vector3 a{ positions[positionsIdx], positions[positionsIdx + 1], positions[positionsIdx + 2] };
			Ogre::Vector3 b{ positions[positionsIdx + 3], positions[positionsIdx + 4], positions[positionsIdx + 5] };
			Ogre::Vector3 c{ positions[positionsIdx + 6], positions[positionsIdx + 7], positions[positionsIdx + 8] };
			Ogre::Vector3 edge1 = b - a;
			Ogre::Vector3 edge2 = c - a;

			data[i++] = edge1.crossProduct(edge2); // cross(edge1, edge2);
		}

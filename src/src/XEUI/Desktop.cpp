...
namespace XE {
	...
	Desktop::Ptr Desktop::Create(GUIRenderer& guiRenderer, CameraRenderable& camera, const XE::Uint16 atlasId) {
		Ptr desktop(new Desktop(guiRenderer, camera, atlasId));
	
		return desktop;
	}
	
	const std::string& Desktop::GetName() const {
		static const std::string name("Desktop");
		return name;
	}
	...
	void Desktop::setSize(sf::Uint32 width, sf::Uint32 height)
	{
		Widget::size = sf::Vector2f(width, height);

		for (const auto& widget : GetChildren())
		{
			widget->ParentPositionChanged();
		}
	}

#include <XEUI/Widgets/Frame.hpp>
...
namespace XE {
	...
	sf::Vector2f Frame::CalculateRequisition() {
		float padding(0); // Context::Get().GetEngine().GetProperty<float>("Padding", shared_from_this()));
	//	const std::string& font_name(Context::Get().GetEngine().GetProperty<std::string>("FontName", shared_from_this()));
//		unsigned int font_size(Context::Get().GetEngine().GetProperty<unsigned int>("FontSize", shared_from_this()));
//		const sf::Font& font(*Context::Get().GetEngine().GetResourceManager().GetFont(font_name));
		float label_padding(0); //(Context::Get().GetEngine().GetProperty<float>("LabelPadding", shared_from_this()));

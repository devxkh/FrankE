...
struct ImDrawList
{
    ...
    inline    void  PrimWriteVtx(const ImVec2& pos, const ImVec2& uv, ImU32 col){ 
		...
		//---------  KH ------------
		float mInvWidth = 1.0f / 800; // mSize.x;
		float mInvHeight = 1.0f / 600; // mSize.y;
		ImVec2 testpos;
		testpos.x = ((pos.x)* mInvWidth) * 2 - 0.99999;
		testpos.y = ((pos.y)* mInvWidth) * 2 - 0.99999;
		
		_VtxWritePtr->pos = testpos;
		
		
		_VtxWritePtr->uv = uv; _VtxWritePtr->col = col; _VtxWritePtr++; _VtxCurrentIdx++; }

...
public:
	...
	bool LoadBuffer(const char *buff, size_t size, const char *name)
	{
		ResetStackOnScopeExit savedStack(_l);
		int status = luaL_loadbuffer(_l, buff, size, name);// "name for error messages");
		...
		const char *msg = lua_tostring(_l, -1);
		_exception_handler->Handle(status, msg ? msg : " dofile failed");
		return false;
	}
